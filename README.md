...
## 고친 것 — `analysis_4` 전환율 팬아웃

조회와 지원을 잇는 키가 이벤트에 없어 `(user_id, job_id)` 로 조인하고 있었다.
같은 사용자가 같은 공고를 여러 번 보면(평균 1.30회) 조회 N × 지원 M 으로 행이 불어난다.

이벤트에 **`view_id`** 를 넣었다. 조회 1회마다 발급하고, 그 조회에서 파생된
북마크·지원이 같은 값을 물고 나간다.

| score_bucket | 수정 전 | 수정 후 | 설계 참값 |
|---|---|---|---|
| 0.6~0.7 | 10.5% | **7.3%** | 7.0% |
| 0.7~0.8 | 10.7% | **6.9%** | 7.0% |
| 0.8~0.9 | 10.9% | **7.1%** | 7.0% |
| 0.9~1.0 | 10.6% | **7.1%** | 7.0% |

이제 설계값(0.20 × 0.35 = 7.0%)과 맞는다.
**여전히 평평한 것은 정상이다** — `match_score` 가 `uniform(0.65, 0.98)` 로 뽑히고
지원 확률과 무관하기 때문이다. 이제는 *올바르게 계산해서* "신호 없음"을 보여준다.

`tests/test_event_schema.py` 7개가 이 불변식을 지킨다 — 고아 이벤트 0,
view_id 당 지원 최대 1회, `(user_id, job_id)` 조인은 실제로 팬아웃이 남는다는 것까지 확인한다.
...
- 스트리밍 지연: 하트비트 1건/초 · 프로브 6회 · 폴링 0.5s (잡은 컨테이너, 프로브는 호스트)
...
- **스트리밍 end-to-end 지연이 선언 상한에 붙어 있다.** Kafka produce → PostgreSQL 적재
  p50 132.5s · 최대 150.1s (프로브 6회, 연속 트래픽). 선언값 합
  (윈도우 30 + 워터마크 60 + 트리거 30×2 = 150s) 위에 배치 실행·적재가 0.5~1.1s 얹혀
  최악 위상에서 상한을 넘는다. 줄이려면 워터마크(60s)를 줄여야 하고 그건 지연 데이터
  허용치와의 교환이다. [docs/metrics.md](docs/metrics.md) 참조
