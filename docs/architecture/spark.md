...
확정된 뒤에야 내보낸다. 즉 30초 윈도우의 결과는 **최소 90초 뒤**에 나온다
(윈도우 30s + 워터마크 60s). 지연 데이터를 버리는 대신 같은 윈도우를 두 번 쓰지 않는다.
`update` 모드였다면 같은 윈도우가 갱신될 때마다 다시 나오므로 append-only INSERT 와 맞지 않는다.
...
- **테이블에 유니크 키가 없다.** `realtime_event_stats` 는 PK 도 제약도 없는 append-only 다.
  체크포인트가 날아가 같은 오프셋을 다시 읽으면 중복 행이 생긴다.
  `(window_start, event_type, is_ai_recommended)` 유니크 + `ON CONFLICT` 가 필요하다.
